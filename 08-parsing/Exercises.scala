...
trait Parsers[ParseError, Parser[+_]]:
  ...
  extension [A](p1: Parser[A]) 
    def map2[B, C](p2: => Parser[B]) (f: (A, B) => C): Parser[C] =
      p1.flatMap{p1a => p2.flatMap{p2b => succeed(f(p1a,p2b))}}
  ...
  def manyA: Parser[Int] = 
    many(char('a')).map{l => l.size}

    //{(p : Parser[Any] => 
    //p.many.map(l => l.size))}
  
  //this.many.map(l => l.size)
  //  {p: Parser[A] => p.map(many)}
  ...
  extension [A](p: Parser[A]) 
    def listOfN(n: Int): Parser[List[A]] = n match
      case 0 => succeed(List[A]())
      case _ => p.map2(p.listOfN(n-1)){_::_}  
  ...
  def digitTimesA: Parser[Int] =
    regex("[0-9]".r)
      .flatMap{i => char('a').listOfN(i.toInt)}
      .map{l => l.size}
