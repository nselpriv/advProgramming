...
object Parsing:
  ...
  lazy val longestLine: Parser[Int] =
    parser.map(lli => lli.map(li => li.length))
    .map(li => li.max)
  ...
  val allLinesTheSame: Parser[Boolean] = 
    parser.map(_.map(_.length).distinct.size == 1)
...
object Game:
  ...
  lazy val Alice: Strategy =
    Pigaro.uniform("strategy Alice")(Move.Paper,Move.Rock,Move.Scissors)

  lazy val Bob: Strategy =
    Pigaro.uniform("Strategy Bob")(Move.Rock,Move.Paper)
  ...
  lazy val aliceFraction: Double = 
    game(Alice,Bob).sample(10000).pr(Some(Player.P1))
